...
/**
 * A variable that must have a real value, or the boot fails.
 *
 * An empty or whitespace-only value counts as absent. That is not pedantry: the
 * usual way a secret goes missing is a `JWT_SECRET=` line or a Kubernetes Secret
 * key with no value, which sets the variable to `""` rather than leaving it
 * unset — and `""` sailed through the `?? fallback` this used to be. For
 * JWT_SECRET that failure is silent where it matters most: `jwt.sign` refuses an
 * empty key, so the admin breaks loudly, but auth/secrets.ts derives the member,
 * Google-state and document-cookie keys through `crypto.hkdfSync`, which accepts
 * zero-length key material happily. Members would keep signing in against keys
 * anyone can recompute from this repository.
 */
function required(name: string, fallback?: string): string {
  const raw = process.env[name];
  const v = raw === undefined || raw.trim() === "" ? fallback : raw;
...
/** Includes restricted sandbox keys generated by Stripe CLI. */
export const stripeTestMode = (): boolean => /^(?:sk|rk|rkcs)_test_/.test(env.stripe.secretKey);
